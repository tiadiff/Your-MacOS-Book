...
const Reader: React.FC<ReaderProps> = ({ page, isRightPage = false }) => {
  const htmlContent = useMemo(() => {
     ...
     try {
       // Fix for layout: 
       // 1. We tell marked to treat single line breaks as <br> (breaks: true).
       // 2. We replace multiple newlines with newlines containing non-breaking spaces.
       //    This forces the parser to recognize them as separate paragraphs/lines 
       //    instead of collapsing them.
       
       // Regex explanation: Match a newline that is immediately followed by another newline.
       // Replace it with a newline + non-breaking space + newline.
       const preservedWhitespace = page.content.replace(/\n(?=\n)/g, '\n&nbsp;\n');
       
       return parse(preservedWhitespace, { breaks: true });
     } catch (e) {
       return page.content;
     }
  }, [page?.content]);
  ...
  return (
    <div className={`h-full w-full bg-paper relative flex flex-col ${isRightPage ? 'shadow-page-left' : 'shadow-page-right'}`}>
      ...
      <div className="flex-1 p-8 md:p-12 overflow-y-auto custom-scroll">
        ...
        <div 
            className="font-serif text-lg leading-loose text-ink prose prose-stone max-w-none prose-headings:font-serif prose-p:font-serif prose-p:min-h-[1em]"
