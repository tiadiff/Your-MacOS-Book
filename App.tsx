...
const App: React.FC = () => {
  ...
  useEffect(() => {
    const applyZoom = () => {
      ...
      // Check if the browser supports the non-standard 'zoom' CSS property (Chrome, Edge, Safari)
      // @ts-ignore
      if ('zoom' in document.body.style) {
         // @ts-ignore
         document.body.style.zoom = zoomLevel;
      } else {
         // Fallback for Firefox and others that don't support zoom
         // We must scale the body and compensate the width/height to fill the viewport
    ...
    // Cleanup function to reset zoom when unmounting (though App usually doesn't unmount)
    return () => {
        // @ts-ignore
  ...
  return (
    <div className="flex flex-col h-screen bg-stone-200 overflow-hidden font-sans">
      ...
      <header className={`h-16 shadow-lg flex items-center justify-between px-6 z-50 transition-colors duration-500`}
        style={{ backgroundColor: activeBook.coverColor === 'brown' ? '#5c3a21' : 
